...
::: {.column width="40%"}
![](assets/anscombe.webp)
:::

::: {.column width="60%"}
![](assets/datasaurus.webp)
:::
...
::: {.notes}
For anything beyond extremely basic plots, base plotting quickly become complex. More importantly, base plots do not have consistency in its functions or plotting strategy.
:::
...
`ggplot2` was built in R by Hadley Wickham in 2005 as an implementation of Leland Wilkinson's book: *Grammar of Graphics*.
:::
...
- Input data is typically an R `data.frame` object
...
data.frame("plot"=c("histogram","smooth","boxplot","density","freqpoly"),
           ...
           "geom"=c("bar","smooth","boxplot","line","line")) |>
...
![](assets/complicated-graphs.webp)
...
Scales are used to control the aesthetics. For example the aesthetic 'color' is mapped to a specific variable. The palette of colors used, the mapping of which color to which value, the upper and lower limit of the data and colors etc is controlled by scales.

![](assets/scales.webp){width="50%"}
...
When setting limits using `scale_`, the data outside the limits are dropped. Limits can also be set using `lims(x=c(3,5))` or `xlim(c(3,5))`. Use `coord_cartesian()` to set limits without dropping data. When mapping, `coord_map()` is recommended for map projections.
...
#| fig-height: 4
#| fig-width: 5
p <- iris |> dplyr::group_by(Species) |> dplyr::tally() |> 
...
The coordinate system defines the surface used to represent numbers. Most plots use the cartesian coordinate system. Pie charts for example, is a polar coordinate projection of a cartesian barplot. Maps for example can have numerous coordinate systems called map projections. For example; UTM coordinates.
...
:::
::: {.column}
![](assets/brain-power.webp)
:::
:::

##

![](assets/meme-plotting-takes-long.webp)
...
#| fig-width: 9
#| echo: false
dfr <- data.frame(value=rep(1,7),label=c("axis.title","axis.text","plot.title","plot.subtitle","legend.text","legend.title","strip.text")) |>
...
#| echo: false
p <- diamonds |>
      filter(cut %in% c("Fair","Good"), color %in% c("D","E")) |>
      droplevels() |>
      ggplot(aes(carat,price,alpha=color,shape=cut))+
            geom_point()+
            labs(title="Title",subtitle="subtitle")+
...
p <- p + theme(
    ...
    panel.border=element_rect(fill=NA,color="#cbd5e8",linewidth=3),
...
#| fig-width: 9
#| echo: false
dfr <- data.frame(value=rep(1,6),label=c("plot.background","panel.background","panel.border","legend.background","legend.box.background","strip.background")) |>
...
newtheme <- theme_bw() + theme(
  ...
  panel.grid.major.y=element_line(linewidth=0.3,color="grey90"), panel.border=element_blank(),
...
```{r}
us <- USArrests |> mutate(state = rownames(USArrests)) |> slice(1:4) |>
      pivot_longer(cols = -state, names_to = "type", values_to = "value")
...
```{r}
#| eval: false
ggsave("plot.png",p,height=5,width=7,units="cm",dpi=200,device=Cairo::CairoPNG)
```
