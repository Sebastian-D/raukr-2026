...
- Typically needs a server (live R environment)
...
- Shiny is built on [Bootstrap](https://getbootstrap.com/docs/3.3/)
...
Regular function `fn <- function()`: Runs wherever it is used  
Reactive function `fn <- reactive()`: Runs only when dependencies change
...
server=function(input,output) {
  getdata <- reactive({ 
  validate(need(input$data_input %in% c("mtcars", "faithful", "iris"), "Please select a valid data set"))
...
```{r}
#| eval: false
fn_validate <- function(x) {
  if (!x %in% c("mtcars", "faithful", "iris")) {
    return("Please select a valid data set")
  }
}
...
server=function(input,output) {
  getdata <- reactive({ 
    validate(fn_validate(input$data_input))
    get(input$data_input,'package:datasets')
  })
  output$table_output <- renderTable({head(getdata())})
})
...
Reactive functions that automatically re-execute whenever its dependencies change. Usually used for side-effects rather than returning a value.

### `observe()`  
Doesn't return a value. Watches for changes in its dependencies as long as the app is alive.
...
### `bindEvent()`
Bind an action to a specific event. Similar to `observeEvent()`, but allows for the event to be determined dynamically. Binds reactive, render, or observer expression to one or more event-like dependencies. Returns a value. A pipe-friendly successor/alternative to `observeEvent()` and `eventReactive()`.

:::{.notes}
Observers are reactive functions that run when their dependencies change similar to regular reactive output functions, affecting HTML output. Observers are used for side-effects.
:::
...
server = function(input,output,session) {
  observe({
    if(something) {
      updateSelectInput(session,"select_input",label="selectInput",choices=c("D","E","F"))
      updateNumericInput(session,"numeric_input",label="numericInput",value=10,min=1,max=10)
    }
  })}
```
...
- Use **bslib** and new Bootstrap versions
...
:::: {.columns}
::: {.column width="40%"}
- Use **shinythemes**, depends on older Bootstrap 3
