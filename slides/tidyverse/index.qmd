...
::: {layout.ncol="3"}
![](assets/hex-tidyverse.webp){height="250px"} ![](assets/Hadley-wickham2016-02-04.webp){height="250px"} ![](assets/RforDataScience.webp){height="250px"}
:::
...
![](assets/tidyverse.webp){height="500px"}\
...
## Introduction to Pipes or Let My Data Flow `r emo::ji('water_wave')` {background-image="assets/pipes.webp" background-opacity="0.8"}

::::: columns
::: {.column width="50%"}
![](assets/MagrittePipe.webp)

[Rene Magritt, [*La trahison des images*](https://en.wikipedia.org/wiki/The_Treachery_of_Images), [Wikimedia Commons](https://en.wikipedia.org/wiki/The_Treachery_of_Images#/media/File:MagrittePipe.webp)]{.smaller}

<br>

![](assets/magrittr.webp)
:::
...
::::: columns
::: {.column width="50%"}
![](assets/T-pipe.webp){height="180px"}
...
From R \>= 4.1.0 we have a native `|>` pipe that is fasterm than `%>%`, dependency-free and native but has less rich placeholders mechanism.
...
a placeholder `_` is available but `r emo::ji('danger')` only for **named** arguments.
...
In `tidyverse` you import data using `readr` package (relying on fast `vroom` engine) that provides a number of useful data import functions:
...
![](assets/diamonds.webp){.center height="200px"}
...
::: callout-note
`rename` is a variant of `select`, here used to rename `x` to `var_x`
:::
...
::: callout-tip
use `relocate()` (instead of legacy `everything()`) to bring some columns to the front
:::

```{r}
#| eval: false
jewels |> relocate(x:z) |> head(n = 2)
jewels |> select(x:z, everything()) |> head(n = 2) # legacy
```
...
And `relocate` is very flexible:
```{r}
#| eval: false
jewels %>%
  relocate(x:z, .after = cut)

jewels %>%
  relocate(x:z, .before = price)
``` 
...
```
should be replaced with modern `across()` approach:
```{r}
#| eval: false
ris |>
  as_tibble() |>
...
- `.groups = "drop"` is an argument to `summarise()`. It tells `summarise()` to return an ungrouped data frame after the summary is computed.
- `ungroup()` is a separate verb that removes grouping from an already grouped data frame.
...
diamonds %>%
  group_by(cut, color) %>%
  summarise(mean_price = mean(price)) |> head(n=2)
```
`summarise()` default `.groups = 'drop_last'` retains grouping by `color`!

### conclusion
- `.groups = "drop"` --> remove all grouping.
- `.groups = "drop_last"` --> remove only the last grouping variable (the default in many cases).
- `.groups = "keep"` --> keep all grouping variables.
- `.groups = "rowwise"` --> return a rowwise data frame.

`.groups = "drop"` is effectively equivalent to calling `ungroup()` immediately after `summarise()`, but **only** in the context of that `summarise()` call. ungroup() is the more general function that can be used anywhere in a `dplyr` pipeline.
:::
...
If some of your column names should be values of a variable, use `pivot_longer` (which modernizes the legacy `gather`):
...
If some of your observations are scattered across many rows, use `pivot_wider` (which modernizes the legacy `spread`):
...
If some of your columns contain more than one value, use `separate_wider_delim` -- the secure, explicit replacement for legacy `separate()`:
...
#| eval: false
jewels4 |>
  separate_wider_delim(dim, delim = "/", names = c("x", "y", "z"))
```
...
::: callout-note
The modern separate_wider_* function family is strict, meaning it will explicitly alert you with an error if rows contain an uneven balance of delimiters!
:::

## Handling Mismatches in wider splits

The modern `separate_wider_delim()` is **strict**. By default, it will throw an error if the delimiter count is inconsistent across rows. Use `too_few` and `too_many` to handle uneven data:
...
jewels4 |> 
  separate_wider_delim(
    ...
    delim = "/", 
    names = c("x", "y", "z"),
    too_few = "align_start",  # Options: "align_start", "align_end", "debug"
    too_many = "drop" # Options: "drop", "merge"
  )
```

**Strict Validation:** No more silent misalignments like in the legacy `separate()`.

**Debug Mode:** Setting `too_few = "debug"` creates temporary logical columns identifying exactly where and why splits failed.
:::
...
If some of your columns contain elements that need to be grouped together:
...
## Missing Data -- the `!is.na()` boilerplate problem

A common pattern in data cleaning:
...
- less boilerplate
- easier to read
- explicit intent: *remove missing rows*

### option 2
Instead of removing rows, sometimes you want to fill missing values:
```{r}
#| eval: false
df |> mutate(x = coalesce(x, y, z))
```
This means *Take the first non-NA value across these columns.*
:::
...
-   `lubridate` for working with dates (now in the `tidyverse` core)
