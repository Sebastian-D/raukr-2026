...
::: {.callout-note}
- Code chunks starting with `shinyApp()` can be simply copy-pasted to an R console and run.  
...
- {{< fa hand-sparkles >}} Interactive app: This can take a few minutes to load.  
:::
...
Input widgets are used to accept content interactively from the user. These widgets usually end in `Input` like `selectInput()`. Below are usage examples of several of Shiny's built-in widgets. Every widget has a variable name which is accessible through `input$` in the server function. For example, the value of a variable named `text_input` would be accessed through `input$text_input`.

:::{.column-page}

```{shinylive-r}
#| label: ui-widgets-input
#| standalone: true
#| components: [editor, viewer]
shinyApp(
ui=fluidPage(
  fluidRow(
    column(6,
           ...
           selectInput("select_input",label="selectInput",choices=c("A","B","C")),
           numericInput("numeric_input",label="numericInput",value=5,min=1,max=10),
           sliderInput("slider_input",label="sliderInput",value=5,min=1,max=10),
           textInput("text_input",label="textInput"),
           textAreaInput("text_area_input",label="textAreaInput"),
           dateInput("date_input",label="dateInput"),
           dateRangeInput("date_range_input",label="dateRangeInput"),
           radioButtons("radio_button",label="radioButtons",choices=c("A","B","C"),inline=TRUE),
...
::: {.callout-warning}
Rendering user-provided text as HTML can be risky. If an app displays raw HTML entered by a user, that user may be able to inject HTML or JavaScript into the page. This can lead to cross-site scripting (XSS) attacks, especially in apps that are shared with other users or store submitted content. Prefer plain text outputs for untrusted input, and only render HTML that you control or have carefully sanitized.
:::
...
![](assets/error-validation-1.webp)
...
![](assets/error-validation-2.webp)

We can use a custom function instead of using `need()`. Below, we have created a function called `fn_validate()` that checks if the input is NULL, NA or an empty string. This is then used in `validate()`.

```{r}
#| label: validation-3
#| eval: false

fn_validate <- function(x) if(is.null(x) | is.na(x) | x=="") return("Input data is incorrect.")
...
server=function(input, output) {
  ...
  getdata <- reactive({
    validate(fn_validate(try(input$data_input)))
    get(input$data_input,'package:datasets')
  })
...
![](assets/error-validation-3.webp)

The last option is to simply hide the error. This may be used in situations where there is no input needed from the user. We use `req()` to check if the input is valid, else stop execution there till the condition becomes true.
...
![](assets/error-validation-4.webp)
...
It is often desirable to let the user download data tables and plots as images. This is done using `downloadHandler()`.
  ...
  server=function(input, output) {
    ...
    output$button_download <- downloadHandler(
      ...
      content = function(file) {
        write.csv(getdata(),file,row.names=FALSE,quote=FALSE)
      })
  },
...
shinyApp(
  ...
  server=function(input, output) {
    ...
    output$text_output <- renderText({
      paste0("Selected dataset: ",input$data_input)
    })
    ...
    output$plot_output <- renderPlot({
      ...
      hist(
        first_col,
        main = paste("Histogram of", input$data_input),
        xlab = col_name
      )
    })
...
- You cannot just install any R package. They need to be compiled specifically for WebR. Only the most popular packages are currently available. To see a list of available packages, go to [https://repo.r-wasm.org/](https://repo.r-wasm.org/).
...
Shinylive has a [Quarto extension](https://quarto-ext.github.io/shinylive/) making it convenient to include shinylive apps into Quarto documents. The interactive Shiny apps in this document were created using this extension.
...
Refer to the shinylive extension [documentation](https://quarto-ext.github.io/shinylive/).
        ...
        output$plot <- renderPlot({
            ggplot(iris, aes(Sepal.Length, Petal.Length, col = Species)) +
                ...
                theme(
                    ...
                    panel.border = element_rect(fill = NA, color = input$in_panel_border, linewidth = 3),
...
- UI layout using pre-defined function (sidebarLayout)
- Input and output widgets and reactivity
...
![](assets/rnaseq-power-preview.webp)
...
```{r}
#| label: rnaseqpower-test
library(RNASeqPower)
RNASeqPower::rnapower(depth=50,cv=0.6,effect=1.5,alpha=0.05,power=0.8)
```
...
```{r}
#| label: rnaseqpower-conditionalui
#| eval: false
#| fig-height: 6
#| fig-width: 8
library(shiny)
...
shinyApp(
  ui=fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
          selectInput("in_pa_est","Variable to estimate",choices=c("n","cv","effect","alpha","power"),selected="n",multiple=FALSE),
          uiOutput("ui_pa")
      ),
      mainPanel(
        verbatimTextOutput("out_pa")
...
```{r}
#| label: rnaseqpower-multiple-input
#| eval: false
#| fig-height: 6
#| fig-width: 8
library(shiny)
...
-   `depth`: A positive numeric
-   `n`: A positive numeric
-   `cv`: A non-negative numeric
-   `effect`: A positive numeric
...
```{r}
#| label: rnaseqpower-input-validation
#| eval: false
#| fig-height: 6
#| fig-width: 8
library(shiny)
...
parse_numbers <- function(x) {
  suppressWarnings(as.numeric(strsplit(gsub("\\s+", "", x), ",")[[1]]))
}

validate_numbers <- function(x, label, min = NULL, max = NULL,
                             min_open = FALSE, max_open = FALSE) {
  validate(
    need(length(x) > 0 && !anyNA(x), paste(label, "must be numeric.")),
    need(
      is.null(min) || if (min_open) all(x > min) else all(x >= min),
      paste(label, "must be", if (min_open) "greater than" else "at least", min, ".")
    ),
    need(
      is.null(max) || if (max_open) all(x < max) else all(x <= max),
      paste(label, "must be", if (max_open) "less than" else "at most", max, ".")
    )
  )
}
...
shinyApp(
  ...
  server=function(input,output){
    ...
    output$out_pa <- renderPrint({
          depth <- parse_numbers(input$in_pa_depth)
          validate_numbers(depth, "Sequencing depth", min = 0, min_open = TRUE)
          
          if(input$in_pa_est != "n") {
            n <- parse_numbers(input$in_pa_n)
            validate_numbers(n, "Sample size", min = 0, min_open = TRUE)
          }
          
          if(input$in_pa_est != "cv") {
            cv <- parse_numbers(input$in_pa_cv)
            validate_numbers(cv, "Coefficient of variation", min = 0)
          }
          
          if(input$in_pa_est != "effect") {
            effect <- parse_numbers(input$in_pa_effect)
            validate_numbers(effect, "Effect", min = 0, min_open = TRUE)
          }
          
          if(input$in_pa_est != "alpha")  {
            alpha <- parse_numbers(input$in_pa_alpha)
            validate_numbers(alpha, "Alpha", min = 0, max = 1, min_open = TRUE, max_open = TRUE)
          }
          
          if(input$in_pa_est != "power")  {
            power <- parse_numbers(input$in_pa_power)
            validate_numbers(power, "Power", min = 0, max = 1, min_open = TRUE, max_open = TRUE)
          }
...
```{r}
#| label: rnaseqpower-set-repo
#| eval: false
setRepositories(c(1,2),graphics=FALSE)
```

This completes this tutorial. Hope you have enjoyed this build and learned something interesting along the way.
