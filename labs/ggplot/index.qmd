...
:::: {.columns}
::: {.column width="50%"}
![](assets/economist-scatterplot.webp)
:::

::: {.column width="50%"}
![](assets/wsj-heatmap.webp)
:::
::::
...
The actual font used on the Economist graph is something close to [ITC Officina Sans](https://www.myfonts.com/collections/officina-sans-font-itc). Since this is not a free font, I am using a free font called [Slabo 27px](https://fonts.google.com/specimen/Slabo+27px). After the font is registered with `showtext`, it can be used in ggplot2 by setting `family = "Slabo 27px"`.
...
- Use `scale_fill_manual` to change the trendline label since it's a fill scale. Compute the model fit first, then use the resulting value to create the legend entry as `R²`.
...
The aim of this challenge is to recreate the plot below originally published in [The Wall Street Journal](http://graphics.wsj.com/infectious-diseases-and-vaccines/). The plot is a heatmap showing the normalized number of cases of measles across US states from 1928 to 2003. X-axis shows years and y-axis shows the names of states. The color of the tiles denote the number of measles cases per 100,000 people. Introduction of the measles vaccine is shown as the black line in 1963.
...
p <- p + geom_segment(x = 1963, xend = 1963, y = 0, yend = 51.5, linewidth = .6, alpha = 0.7) +
  annotate("text",
