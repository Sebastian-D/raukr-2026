...
We'll do this one on the command line regardless of whether you've been using
Positron, because seeing the raw conflict markers is the whole point.
...
::: {.callout-note}
Positron's conflict editor shows the same thing with "Accept Current / Accept
Incoming / Accept Both" buttons. Now that you've seen the markers by hand, those
buttons should make sense — they're just editing out the `<<<`/`===`/`>>>` for
you.
:::
